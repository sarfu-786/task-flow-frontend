...
export const DeleteOpportunityModal = () => {
  const { isDeleteModalOpen, opportunityToDelete, closeDeleteModal, deleteOpportunity } = useOpportunities();
  const [isDeleting, setIsDeleting] = useState(false);
  const [serverError, setServerError] = useState('');

  if (!isDeleteModalOpen || !opportunityToDelete) return null;
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setServerError(err.message || 'Failed to delete opportunity');
    } finally {
      setIsDeleting(false);
    }
  };
      ...
      >
        ...
        <div className="modal-body" style={{ padding: '20px' }}>
          <p style={{ margin: '0 0 12px', fontSize: '0.9rem', color: 'var(--text-secondary)' }}>
            Are you sure you want to delete opportunity <strong>"{opportunityToDelete.name}"</strong>
            {opportunityToDelete.company ? ` (${opportunityToDelete.company})` : ''}?
          </p>
          <p style={{ margin: 0, fontSize: '0.8rem', color: 'var(--text-muted)' }}>
            This will permanently remove this deal and its pipeline value (₹{opportunityToDelete.amount?.toLocaleString('en-IN')}) from your sales metrics.
          </p>
        </div>
